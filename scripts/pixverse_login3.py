...
async def main():
    async with async_playwright() as p:
        ...
        # 关弹窗 - 找所有 X 按钮
        try:
            close = page.locator('button[aria-label="Close"], [aria-label="Close"], button:has-text("✕"), .fixed button, .absolute button').first
        ...
        try:
            ...
            await google.click(timeout=5000, force=True)
            print("点击Google登录")
            await asyncio.sleep(3)
        except Exception as e:
            print(f"找Google按钮: {e}")
        ...
        if "accounts.google" in page.url:
            ...
            try:
                await page.locator('input[type="email"], input[name="identifier"], input[aria-label*="email"]').first.fill(EMAIL)
                await asyncio.sleep(1)
